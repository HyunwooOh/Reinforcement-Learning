...
class DRQN():
    def __init__(self, args, action_size, rnn_cell, myScope):
        self.scalarInput = tf.placeholder(shape=[None, Config.IMAGE_HEIGHT*Config.IMAGE_WIDTH], dtype=tf.float32)
        self.imageIn = tf.reshape(self.scalarInput, shape=[-1, Config.IMAGE_HEIGHT, Config.IMAGE_WIDTH, 1])
        ...
        self.conv4 = tf.contrib.layers.convolution2d(inputs=self.conv3, num_outputs=Config.DRQN_HSIZE, kernel_size=7, stride=1, padding='VALID', scope=myScope+'_conv4')
        self.unrolling_time_steps = tf.placeholder(dtype=tf.int32, shape=[])
        self.batch_size = tf.placeholder(dtype=tf.int32, shape=[])
        self.convFlat = tf.reshape(tf.contrib.layers.flatten(self.conv4), [self.batch_size, self.unrolling_time_steps, Config.DRQN_HSIZE])
        ...
        self.rnn = tf.reshape(self.rnn, shape=[-1, Config.DRQN_HSIZE])
        ...
        self.trainer = tf.train.AdamOptimizer(learning_rate=Config.ADAM_LEARNING_RATE)
...
def train(args):
    ...
    cell = tf.contrib.rnn.BasicLSTMCell(num_units=Config.DRQN_HSIZE, state_is_tuple=True)
    cellT = tf.contrib.rnn.BasicLSTMCell(num_units=Config.DRQN_HSIZE, state_is_tuple=True)
    main = DRQN(args, ACTION_SIZE, cell, 'main')
    target = DRQN(args, ACTION_SIZE, cellT, 'target')
    ...
    with tf.Session(config=config) as sess:
        ...
        for episode in range(1, 99999999):
            ...
            if global_steps > Config.TRAIN_END: sys.exit()
